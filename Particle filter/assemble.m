...
length_d=length(d);
while(k<=length_d)
    New_Sample_Set(d(k)).x=Sample_Set(d(k)).x; %#ok<AGROW>
    New_Sample_Set(d(k)).y=Sample_Set(d(k)).y; %#ok<AGROW>
    k=k+1;
end
...
for i=1:length_c-1
    for j=i+1:length_c
        k=c(i);
        if(usetimes(c(j))>usetimes(k))
            k=c(j);
        end
    tmp = usetimes(k);
    usetimes(k)=usetimes(c(i));
    usetimes(c(i))=tmp;
    end
end
...
while(i<=length_c)
    while(usetimes(c(i))>1&&j<=length_b)
        ...
        New_Sample_Set(b(j)).x=Sample_Set(c(i)).x;
        New_Sample_Set(b(j)).y=Sample_Set(c(i)).y;
        j=j+1;
...
%新样本中每个粒子的权值重新分配为1/N
for i=1:N
    New_Sample_probability(i)=1/N; %#ok<AGROW>
end